...
import { StageModel, GroupStageModel, KnockoutStageModel, StageType } from '../../../../libs/db/stage.schema';
import { StageDto, GroupStageRulesDto, KnockoutStageRulesDto } from './dto/stage.dto';

@Injectable()
export class StageService {
  constructor(
    @InjectModel('Stage') private readonly stageModel: Model<any>,
  ) {}

  async create(dto: StageDto): Promise<StageDto> {
    let created;
    if (dto.type === StageType.Group) {
      created = await GroupStageModel.create({
      ...
      created = await KnockoutStageModel.create({
        seasonId: dto.season?.id,
        order: dto.order,
        name: dto.name,
        type: dto.type,
  ...
  async findById(id: string): Promise<StageDto> {
    ...
    if (stage?.type === StageType.Group && stage.teamGroups) {
      for (const groupName of Object.keys(stage.teamGroups)) {
        stage.teamGroups[groupName] = await this.stageModel.db.model('Team').find({ _id: { $in: stage.teamGroups[groupName] } }, 'name').lean();
      }
    }
